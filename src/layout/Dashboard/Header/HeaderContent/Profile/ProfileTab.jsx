...
export default function ProfileTab() {
  ...
  const navigate = useNavigate(); // Use the router for navigation

  const handleLogout = () => {
    // Clear user session data
    localStorage.removeItem('userToken'); // Adjust based on how you store the session
    localStorage.removeItem('userRole'); // Remove any other relevant data
    navigate('/login');
  };
  ...
  return (
    <List component="nav" sx={{ p: 0, '& .MuiListItemIcon-root': { minWidth: 32 } }}>
      ...
      <ListItemButton
        ...
        onClick={() => {
          setSelectedIndex(2); // Update selected index if needed
          handleLogout();
        }}
      >
        <ListItemIcon>
